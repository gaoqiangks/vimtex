...
function! vimtex#env#toggle_star() abort
    ...
    function! s:operator_setup(operator, type) abort
        ...
        if s:operator ==# 'change'
            ...
            local env_old =   vim.api.nvim_eval("g:env_name")
            local final_env = env.get_env_from_old(env_input, env_old)
            --print("final_env", final_env)
            vim.g.new_env = final_env
            --vim.api.nvim_exec('let g:new_env = luaeval("final_env")', true)
EOF
            " echo "g:new_env=3 ".g:new_env
